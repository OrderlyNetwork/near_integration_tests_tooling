...
/// Initialize context for integration test
/// Create contract, test accounts and tokens
/// Mint tokens to test accounts according to TestAccount.mint_amount fields
///
/// # Arguments
/// * `token_infos` - TokenInfo array, that contains token contract info and wasm file
/// * `test_accounts` - TestAccount array, that contains test account id and mint amounts
/// * `contract_initializer` - ContractInitializer, that contains contract id, wasm file and role accounts
///
/// # Returns
/// * `Worker<Sandbox>` - workspaces worker
/// * `T` - test contract instance, that able to call contract methods, generated by test bindgen macro
/// * `U` - contract holder instance, that contains initialized role accounts
/// * `[TokenContractTest; N]` - initialized test token contracts, for ft_transfer ft_transfer_call calls
/// * `[Account; M]` - initialized test accounts, with minted tokens
...
/// Generate accounts if there are more than `JOIN_MAX` accounts.
/// This is done to avoid spawning too many tasks at once.
/// That can cause error `Too many open files` on some systems.
...
/// Make storage deposits for account in token contract
...
/// Mint tokens for account in token contract
...
/// Make storage deposits and mint tokens for all accounts
